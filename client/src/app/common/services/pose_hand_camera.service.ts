...
export class WebCamSkeletonService {
  ...
  private currentHandResultsSubject: BehaviorSubject<any>;
  public currentHandResults$: Observable<any>;
  ...
  constructor() {
    ...
    // ✅ Initialize hands results subject
    this.currentHandResultsSubject = new BehaviorSubject<any>(null);
    this.currentHandResults$ = this.currentHandResultsSubject.asObservable();
  }
  ...
  detectPoseInRealTime = async (videoElement, net, canvasId = 'patient-canvas') => {
    ...
    if (this.handNet) {
      this.handNet.onResults(this.onHandResults);
    }
  ...
  throttleSkeleton = async (data) => {
    // Run pose first to keep body detection responsive, then hands
    if (this.net) await this.net.send(data);
    if (this.handNet) await this.handNet.send(data);
  };
  ...
  onHandResults = (results) => {
    // Emit hand landmarks and draw hands; forward image to pose if needed
    if (results && results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
      this.currentHandResultsSubject.next(results.multiHandLandmarks);
      this.drawHands(results);
    } else {
      if (results && results.image) {
        // Run pose on the same frame when hands not detected this pass
        this.net && this.net.send({ image: results.image });
      }
    }
  };
  ...
  drawHands = (results) => {
    ...
    for (const landmarks of results.multiHandLandmarks) {
      drawLandmarks(ctx, landmarks, { color: 'cyan', lineWidth: 2 });
    }
    ctx.restore();
  };
  ...
  initializeModel = async () => {
    if (!this.net) {
      this.net = new Pose({
        // Load Pose model assets from CDN to avoid local wasm issues
    ...
    if (!this.handNet) {
      this.handNet = new Hands({
        // Load Hands model assets from a known published version
        locateFile: (file) => `https://cdn.jsdelivr.net/npm/@mediapipe/hands@0.4.1646424915/${file}`,
      });
      this.handNet.setOptions({
        selfieMode: true,
        maxNumHands: 2,
        modelComplexity: 1,
        minDetectionConfidence: 0.7,
